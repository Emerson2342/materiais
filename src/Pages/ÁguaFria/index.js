...
export default function AguaFria() {
  ...
  const removerItem = (indexToRemove) => {
    ...
    const novoArray = aguaFria.filter((_, index) => index !== indexToRemove);

    // Atualizar o estado com o novo array
    setAguaFria(novoArray);
  };

  const addAoCarrinho = (index) => {
    const item = aguaFria[index];
  ...
  const renderItem = ({ item, index }) => (
    <View style={styles.aguaFriaContainer}>
